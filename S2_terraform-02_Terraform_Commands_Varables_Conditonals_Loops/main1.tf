...
resource "aws_instance" "tf-ec2" {
  ...
  key_name      = "clarusway"
  tags = {
    "Name" = "${local.mytag}-come from locals"   # "${var.ec2_name}-instance"     # local eklendigi icin bu satir yorm satiri olarak alindi. # cc
}
}

resource "aws_s3_bucket" "tf-s3" {
  bucket = "${var.s3_bucket_name}-${count.index + 1}"   # best practice bu uygulanir. Bunu yazmaz isen terraform tfstate dosyasinda 0'dan baslayarak bir numara veriyor. Bu sekilde dha duzenli oluyor.
